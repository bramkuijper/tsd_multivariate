    ...
    metapopulation(par.npatches,Patch(par))
{
            ...
            time_step <= par.max_simulation_time; ++time_step)
    {
        ...
        if (time_step % par.max_t_season == 0)
        {
            adult_survival();
            fill_vacancies();
            replace();
...
void TSDSeasonal::reproduce()
{
            ...
            ++patch_idx)
    {
                ...
                ++female_idx)
        {
            ...
            time_threshold_diff = metapopulation[patch_idx].
                females[female_idx].time_threshold - 
                time_threshold_i;
            ...
            if (static_cast<int>(time_step) % par.max_t_season >= time_threshold_i)
            {
                        ...
                        egg_idx < par.fecundity; ++egg_idx)
                {
                    ...
                    double p_female = Kid.determine_sex(temperature_this_female);
                    ...
                    {
                        if (uniform(rng_r) < 
                                calculate_survival(female, temperature_this_female))
                        {
                    ...
                    {
                        if (uniform(rng_r) < 
                                calculate_survival(male, temperature_this_female))
                        {
...
// calculate survival
double TSDSeasonal::calculate_survival(
        Sex const the_sex,
        double const local_temperature)
{
    return(std::exp(-0.5 * 
                (par.t_opt[the_sex] - local_temperature) * 
                (par.t_opt[the_sex] - local_temperature) / 
                    par.omega_t[the_sex]));
...
void TSDSeasonal::write_parameters()
{
    data_file << std::endl << std::endl
        ...
        << "n;" << par.n << std::endl
        << "skip;" << par.skip_output << std::endl
        << "fecundity;" << par.fecundity << std::endl
        ...
        << "init_depth;" << par.init_depth << std::endl
...
void TSDSeasonal::write_data()
{
    ...
    double mean_depth_slope{0.0};
    double ss_depth_slope{0.0};
    ...
    mean_depth /= nf + nm;
    mean_depth_slope /= nf + nm;

    double var_depth = ss_depth / (nf + nm) - 
